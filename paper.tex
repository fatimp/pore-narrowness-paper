...
rotating an image of a pore or scaling it by a factor $0.5-2$. This inaccuracy
is OK for assessing quality of pore clusterization, as can be seen in the next
section.
\begin{figure}
  \centering
...
\begin{table*}[!htp]
  ...
  \begin{tabular}{|c|c|}
    ...
    \hline
  \end{tabular}
  \caption{Different values of \highlight{awesomeness} and their meaning.}
  \label{tab:ranges}
\end{table*}
...
which does not cross the pore's boundary. Although the resulting algorithm will
have the same $O(n^2)$ complexity, doing so can increase computational time
significantly. We call the parameter $P'$ with $d_{AB}$ in the place of
$\rho_{AB}$ ``improved awesomeness''.
